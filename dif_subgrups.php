...
if(isset($subgrup)&&($subgrup!='')&&isset($esborrar_curs)&&($esborrar_curs!='')) {
  $subgru=split(' ',$subgrup);
		...
		//borrem la relació
		$query = "DELETE FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE subgrup_orig='$subgru[0]' AND curs = '$esborrar_curs'";
...
//   echo "<p> Query: $consulta </p> \n";
  ...
  // alumnes a esborrar
  $query = "SELECT E.numero_mat FROM $bdtutoria.$tbl_prefix"."Estudiants_Materies EM, $bdtutoria.$tbl_prefix"."Estudiants E WHERE EM.numero_mat=E.numero_mat AND EM.codi_credit='$subgrup_orig' AND concat(E.curs, ' ', E.grup, ' ', E.pla_estudi)='$esborrar_curs'";
...
if(isset($subgrup)&&($subgrup!='')&&isset($esborrar_credit)&&($esborrar_credit!='')) {
  $subgru=split(' ',$subgrup);
	...
	$query = "DELETE FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE ref_subgrup='$subgru[0]' and subgrup_orig = '$esborrar_credit'";
	if( !mysql_query($query, $connect) ) {
		echo "<p> Query: $query </p> \n";
		print("<br>Error: ". mysql_error() .".<br>");
	}
...
//   echo "<p> Query: $consulta </p> \n";
  ...
  // alumnes a afegir, si cal
  $query = "SELECT E.numero_mat FROM $bdtutoria.$tbl_prefix"."Estudiants_Materies  WHERE codi_credit='$esborrar_credit'";
  $res = mysql_query( $query );
  while( $fila=mysql_fetch_row($res) )
	$afegir_array[] = $fila[0];
		// alumnes candidats 
	$query = "SELECT curs FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE ref_subgrup = '$subgru[0]'";
	$res = mysql_query( $query );
	while( $fila=mysql_fetch_row($res) ) {
		$query2 = "SELECT numero_mat FROM $bdtutoria.$tbl_prefix"."Estudiants  WHERE concat(E.curs, ' ', E.grup, ' ', E.pla_estudi)='$fila'";
		$res2 = mysql_query( $query2 );
  		while( $fila2=mysql_fetch_row($res2) )
			$candidats_array[] = $fila2[0];
		mysql_free_result($res2);
	}
	mysql_free_result($res);

  $nouconjunt = implode(',', array_intersect($afegir_array, $candidats_array));
		// els juntem
	$final_array = array_merge( $alssubgrup, $nouconjunt );
	$final_array = array_unique( $final_array );
	$alsfinals = implode(',', $final_array );

  // actualitzem
	$query = "UPDATE $bdtutoria.$tbl_prefix"."subgrups SET alumnes = '$alsfinals' WHERE ref_subgrup='$subgru[0]'";
	if( !mysql_query($query, $connect) ) {
		echo "<p> Query: $query </p> \n";
		print("<br>Error: ". mysql_error() .".<br>");
	}
...
if(isset($subgrup)&&($subgrup!='')&&isset($afegir_curs)&&($afegir_curs!='')) {
  $subgru=split(' ',$subgrup);
		//afegim la relació
	$query = "INSERT INTO $bdtutoria.$tbl_prefix"."dif_subgrups (ref_subgrup, curs, subgrup_orig) VALUES ('$subgru[0]', '$afegir_curs', '')";
	if( !mysql_query($query, $connect) ) {
		echo "<p> Query: $query </p> \n";
		print("<br>Error: ". mysql_error() .".<br>");
	}
...
// 		echo "<p> Query: $consulta </p> \n";
	...
	$nous_array = Array('');
	$i=0;
	$query = "SELECT E.numero_mat FROM $bdtutoria.$tbl_prefix"."Estudiants E WHERE concat(E.curs, ' ', E.grup, ' ', E.pla_estudi)='$afegir_curs'";
 		echo "<p> Query: $query </p> \n";
	$res = mysql_query( $query );
	while( $fila=mysql_fetch_row($res) ) {
		$nous_array[$i++] = $fila[0];
		echo "<p> fila 0: $fila[0]</p>\n";
		echo "<p> i: $i</p>\n";
		printf( $nous_array );
	}
	mysql_free_result($res);
	printf($nous_array);	
		// alumnes a restar 
	$query = "SELECT subgrup_orig FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE ref_subgrup = '$subgru[0]'";
	$res = mysql_query( $query );
	while( $fila=mysql_fetch_row($res) ) {
		$query2 = "SELECT E.numero_mat FROM $bdtutoria.$tbl_prefix"."Estudiants_Materies  WHERE codi_credit='$fila'";
		$res2 = mysql_query( $query2 );
  		while( $fila2=mysql_fetch_row($res2) )
			$esborrar_array[] = $fila2[0];
		mysql_free_result($res2);
	}
	mysql_free_result($res);
	printf($esborrar_array);	
	$nous_array = array_diff($nous_array, $esborrar_array);
	printf($nous_array);	
...
if(isset($subgrup)&&($subgrup!='')&&isset($afegir_credit)&&($afegir_credit!='')) {
  $subgru=split(' ',$subgrup);
		//afegim la relació
	$query = "INSERT INTO $bdtutoria.$tbl_prefix"."dif_subgrups (ref_subgrup, curs, subgrup_orig) VALUES ('$subgru[0]', '', '$afegir_credit')";
	if( !mysql_query($query, $connect) ) {
		echo "<p> Query: $query </p> \n";
		print("<br>Error: ". mysql_error() .".<br>");
	}
...
// 		echo "<p> Query: $consulta </p> \n";
  ...
  // alumnes a esborrar
  $query = "SELECT E.numero_mat FROM $bdtutoria.$tbl_prefix"."Estudiants_Materies  WHERE codi_credit='$afegir_credit'";
  $res = mysql_query( $query );
  while( $fila=mysql_fetch_row($res) )
	$esborrar_array[] = $fila[0];
...
if(isset($subgrup)&&($subgrup!='')&&isset($esborrarsubgrup)&&($esborrarsubgrup!='')) {
  ...
  mysql_free_result($conjunt_resultant);
	...
	//borrem les relacions
	$query = "DELETE FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE ref_subgrup='$subgru[0]'";
	if( !mysql_query($query, $connect) ) {
		echo "<p> Query: $query </p> \n";
		print("<br>Error: ". mysql_error() .".<br>");
	}
...
if ($subgrup!='') {
  ...
  $subgru=split(' ',$subgrup);
    ...
    $consulta="SELECT curs FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE ref_subgrup='$subgru[0]'";
// 	echo "<p> Query: $consulta </p> \n";
    $conjunt_resultant=mysql_query($consulta, $connect);
	...
	/////////////// subgrups ///////////////////////
  if( empty($subgrup_orig) ) {
	$consulta="SELECT subgrup_orig FROM $bdtutoria.$tbl_prefix"."dif_subgrups WHERE ref_subgrup='$subgru[0]'";
// 		echo "<p> Query: $consulta </p> \n";
	$conjunt_resultant=mysql_query($consulta, $connect);
	if( mysql_num_rows($conjunt_resultant) > 0 )
		$subgrup_orig = mysql_result( $conjunt_resultant, 0, 0 );
  	mysql_free_result($conjunt_resultant);
  }
  print("<input type='hidden' name='subgrup_orig' value='$subgrup_orig'>");

  if( empty($subgrup_orig) ) print("Aquest subgrup no t&eacute; subgrup complementari.");
  else {  
    print("<table border='0'>");
    print("<tr bgcolor='#aacccc'><td><a href='' title='Clica per esborrar-lo del subgrup.' onClick='document.introd1.esborrar_subgrup.value=\"$fila[0]\"; document.introd1.submit(); return false;'>$subgrup_orig</a></td></tr>");
    print("</table>");
  }
  print("<hr>");
	/////////////// alumnes del subgrup ///////////////////////
...
<?php
  ...
  foreach( $llista_grups as $curs ) {
    if (isset($subgrup)&&$subgrup!='') {
      $nohies=true;
      foreach($cursos_subgrup as $curs_subgrup) 
			if ($curs_subgrup==$curs) 
				$nohies=false;
      if($nohies) {
	  		print("
		<tr bgcolor='#aacccc'>
			<td><a href='#' title='Clica per afegir-lo al subgrup.' onClick='document.introd1.afegir_curs.value=\"".$curs."\"; document.introd1.submit(); return false;'>$curs</a></td>
		</tr>"
      	  );
      } // if nohies
    } // if isset subgrup
  ...
  $consulta="SELECT count(*) FROM $bdalumnes.$tbl_prefix"."subgrups";
  $conjunt_resultant=mysql_query($consulta, $connect);
  ...
  $paginador.= "<";
  $paginador.= ($paginadorenrere)?"</a>":"";
  $paginador.= "&nbsp; &nbsp; Subgrups ".($paginadoractual+1)." - ".((($paginadoractual+$maxpaginador)<=$nregs)?($paginadoractual+$maxpaginador):$nregs)."&nbsp; &nbsp; ";
