...
<td>");
    ...
    if(!isset($editaavaluacio)) {
	    ...
	    print("<a id='linknovaaval' href='' onClick='var refaval=prompt(\"Referència de la nova avaluació (màx. 10 caracters, A-Za-z0-9. , sense espais, la referència d´avaluació no pot estar repetida.)\",\"--- Introdueix aqui la referència de la nova avaluació ---\");   if(refaval!=\"\" && refaval!=null && refaval==refaval.match(/[a-z0-9.]+/gi  ) ) {document.getElementById(\"linknovaaval\").href=\"$PHP_SELF?idsess=$idsess&afegiravaluacio=si&refaval=\"+refaval; return confirm(\"Es crearà una nova avaluació de nom: \"+refaval+\" . Amb valors dels paràmetres per defecte que es podran editar posteriorment.\");} else {if(refaval!=null) alert(\"Referència incorrecta.\"); return false;}  '>Afegir nova avaluaci&oacute;</a><br><br>");
