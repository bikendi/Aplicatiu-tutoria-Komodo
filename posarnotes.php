...
<div align='right'>
<table border='0'>
<tr><td valign='bottom'><input type='button' value='Imprimir' onClick='window.print();'>&nbsp; &nbsp; </td><td><font size='6'>Qualificacions&nbsp; &nbsp; </font></td>
<td align='right'>
Grup/Subgrup: ");
...
if((isset($grup)&&$grup!="") || (isset($subgrup)&&$subgrup!="")) {
  print("<br>Assignatura/Credit: <select name='assign' onChange='document.introd1.submit();'><option></option>");
...
if ($grup!="" || $subgrup!="") {
  ...
  if($nregs!=0) {
    ...
    $conjunt_resultant=mysql_query($consulta, $connect); //restauro consulta
    $consulta1.=$aux1.") ";
    $aux1=explode(' ',$assign);
    $codassign=$aux1[0];
    ...
    while($fila1=mysql_fetch_row($conjunt_resultant1)) {
       if($assign!='' && $hihaassignselected) {
	    $consulta2="select nomitems, nomaval, data from $bdtutoria.$tbl_prefix"."avaluacions where refaval='$fila1[0]' limit 1";
	    $conjunt_resultant2=mysql_query($consulta2, $connect);
	    $fila2=mysql_fetch_row($conjunt_resultant2);
	    $aux=explode('|', $fila2[0]);	       
		$capcal .= "<td align='center' title='".$nomDiaSem[date('w',$fila2[2])].", ".date('j-n-Y',$fila2[2])." - ".rawurldecode($fila2[1])."'><b>".$codassign."<br>".$fila1[0]."</b>";
		if($fila2[0]!='') {
